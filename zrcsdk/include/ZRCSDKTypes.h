...
enum ThirdPartyMeetingServiceProvider
{
    ...
    ThirdPartyMeetingServiceProviderTencent = 9            ///<Tencent
};
...
struct MeetingInfo
{
    ...
    bool isParticipantLocalTimeEnabled = false;      ///<TRUE indicates the meeting supports showing participant local time.
    bool isShowOnlyDifferentTimezoneEnabled = false; ///<TRUE indicates only show local time for participants in different timezones.
};
...
struct MeetingParticipant
{
    ...
    int32_t               timeZoneOffsetMinutes = 0;                  ///<Participant's timezone offset in minutes from UTC.
    bool                  isSupportDisplayLocalTime = false;          ///<TRUE indicates participant supports displaying local time.
};
...
/*! \enum ProAVLayoutAlign
    \brief Pro AV layout alignment mode.
    Here are more detailed structural descriptions.
 */
enum ProAVLayoutAlign
{
    ProAVLayoutAlignCenter       = 0,   ///<Tiles are packed towards the center.
    ProAVLayoutAlignStart        = 1,   ///<Tiles are packed towards the start of the axis.
    ProAVLayoutAlignEnd          = 2,   ///<Tiles are packed towards the end of the axis.
    ProAVLayoutAlignSpaceBetween = 3,   ///<Tiles are evenly distributed with equal space between tiles.
    ProAVLayoutAlignSpaceEvenly  = 4    ///<Tiles are evenly distributed with equal space from edges.
};

/// \brief Pro AV layout settings
///
struct ProAVLayoutSettings
{
    float aspectRatio = 16.0/9.0;                               ///<Aspect ratio for individual video feeds. Default: 16.0 / 9.0.
    uint32_t maxTileWidth = 0;                                  ///<Maximum width in pixels. 0 means no fixed width limit.
    uint32_t maxTileHeight = 0;                                 ///<Maximum height in pixels. 0 means no fixed height limit.
    ProAVLayoutAlign alignHorizontal = ProAVLayoutAlignCenter;  ///<Horizontal alignment of tiles. Default: ProAVLayoutAlignCenter.
    ProAVLayoutAlign alignVertical = ProAVLayoutAlignCenter;    ///<Vertical alignment of rows. Default: ProAVLayoutAlignCenter.
    uint32_t tileSpacing = 0;                                   ///<Spacing between tiles in pixels. Valid for alignment is Center, Start or End.
    uint32_t canvasOutsidePaddingLeft = 0;                      ///<Additional padding between the tiles and the left edge of the canvas.
    uint32_t canvasOutsidePaddingRight = 0;                     ///<Additional padding between the tiles and the right edge of the canvas.
    uint32_t canvasOutsidePaddingTop = 0;                       ///<Additional padding between the tiles and the top edge of the canvas.
    uint32_t canvasOutsidePaddingBottom = 0;                    ///<Additional padding between the tiles and the bottom edge of the canvas.
    uint32_t canvasBackgroundArgb = 0;                          ///<Canvas background color, ARGB, 8 bits per channel. Default value: 0x0 (black, opaque). The alpha value is currently ignored and can't be used for root canvas background transparency. Bit representation: (msb) aaaaaaaa|rrrrrrrr|gggggggg|bbbbbbbb (lsb)
};

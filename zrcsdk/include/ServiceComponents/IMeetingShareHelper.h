...
/// \brief HDMI share resolution and frame rate option.
///
struct HDMIShareResolutionAndFrameRateOption
{
    uint32_t    type = 0;               ///<Resolution and frame rate type identifier.
    std::string typeDescription;        ///<Description of the resolution and frame rate type (e.g., "4K 30fps").
    bool        isDisabled = false;     ///<TRUE indicates this option is currently disabled.
};
...
public:
     ...
     * @zrminver 7.0.0
     *
     * @brief HDMI share resolution and frame rate options notification.
     *
     * This callback is triggered when the available HDMI share resolution and frame rate options change
     * or the currently selected option changes.
     *
     * @param selectionList Available resolution and frame rate options.
     * @param selectedType The currently selected resolution and frame rate type.
     */
...
public:
     ...
     * Otherwise failed. To get extended error information, see \link ZRCSDKError \endlink enum.
     *
     * @deprecated Since 7.0.0. Use SetHDMIShareResolutionAndFrameRate instead.
     */
     ...
     * @zrminver 7.0.0
     *
     * @brief Set HDMI share resolution and frame rate.
     *
     * Unified API that works transparently across all ZR versions.
     * Use the type value from the selection list provided by OnHDMIShareResolutionAndFrameRateNotification.
     *
     * @param type The resolution and frame rate type to set.
     * @return If the function succeeds, the return value is @ref ZRCSDKERR_SUCCESS.
     * Otherwise failed. To get extended error information, see \link ZRCSDKError \endlink enum.
     */
    virtual ZRCSDKError SetHDMIShareResolutionAndFrameRate(uint32_t type) = 0;
